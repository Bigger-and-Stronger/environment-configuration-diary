...
void load_cross_field(
    const std::string& filename,
    Eigen::MatrixXd& field1,
    Eigen::MatrixXd& field2)
{
  std::ifstream file(filename);
    if (!file.is_open()) {
        std::cerr << "Error: Could not open cross field file " << filename << std::endl;
        exit(EXIT_FAILURE);
    }

    std::vector<Eigen::RowVector3d> data1, data2;
    std::string line;
    int line_num = 0;
    while (std::getline(file, line)) {
        ...
        if (!(ss >> v1x >> v1y >> v1z >> v2x >> v2y >> v2z)) {
            std::cerr << "Error: Could not parse line " << line_num << " in file " << filename << std::endl;
            exit(EXIT_FAILURE);
        }

        data1.push_back(Eigen::RowVector3d(v1x, v1y, v1z));
        data2.push_back(Eigen::RowVector3d(v2x, v2y, v2z));
    }
    file.close();

    if (data1.empty()) {
        std::cerr << "Error: File " << filename << " is empty or has incorrect format." << std::endl;
        exit(EXIT_FAILURE);
    }

    field1.resize(data1.size(), 3);
    field2.resize(data2.size(), 3);

    for (size_t i = 0; i < data1.size(); ++i) {
        field1.row(i) = data1[i];
        field2.row(i) = data2[i];
    }
    
    std::cout << "Successfully loaded " << field1.rows() << " cross field vectors from " << filename << std::endl;
}
...
int main(int argc, char *argv[])
{
  ...
  igl::readPLY("/home/yuxiaoyang/NeurCross/data/doubleTorus/input/doubleTorus.ply", V, F);
  ...
  global_scale =  .5*igl::avg_edge_length(V, F);
    ...
    // Read cross field from NeurCross
    load_cross_field("/home/yuxiaoyang/NeurCross/quad_mesh/doubleTorus/doubleTorus/save_crossField/doubleTorus_iter_9999.txt", X1, X2);
...
igl::copyleft::comiso::miq(V,
  ...
  igl::writeOBJ("doubleTorus_neurcross.obj", V, F, Eigen::MatrixXd(), Eigen::MatrixXi(), UV, FUV);
  std::cout << "  - Saved parameterized 3D mesh to: parameterized_mesh.obj" << std::endl;
